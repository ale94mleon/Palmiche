...
import os, datetime, tempfile, argparse, time, multiprocessing, socket
import numpy as np
import pandas as pd
    ...
    while True:
        ...
        if os.path.isfile(log_center_cluster_abspath):
            ...
            if time_progression[-1] >= simulated_tempering_weights_equilibration_time:
                ...
                nextjobs2launch = {}
                for frame in user_membership[current_cluster]:
                    ...
                    nextjobs2launch[ID_dir_abspath_member] = {'cluster': current_cluster, 'window_ID':window_ID}
                ...
                print(f'we are launching inside of the setup_production() the production job for the corresponded cluster memebers. Jobs = {list(nextjobs2launch.keys())}')
                if frontend:
                    tools.job_launch_list([os.path.join(path, '3_job.sh') for path in nextjobs2launch], shell='bash')# This is just for testing
                else:
                    ...
                    tools.launch_wait_check_repeat(
                        partition = partition,
                        jobpaths = list(nextjobs2launch.keys()),
                ...
                print(f'The launching inside of the setup_production() the production of the Jobs = {list(nextjobs2launch.keys())} was succesed.')
                break
...
def main(input_path_dict,
    ...
    if window_code:
        ...
        # Here We calculate the fingerprint if simulated tempering is turn on and simulated_tempering_n_clusters is set to a number different to zero.
        ...
        print('Selection the frames for each window')
        windows = {}
        ...
        if simulated_tempering:
            ...
            if simulated_tempering_n_clusters:
                ...
                for window_ID in windows:
                    ...
                    user_membership[0].append(windows[window_ID]['frame'])
                window_ID = 1
                frame_close_to_centroids = [windows[window_ID]['frame']]
                user_membership[cluster].remove(frame_close_to_centroids[0])
        
        print('Saving window configuration data')
        ...
        if simulated_tempering:
            ...
            cluster_process = {}
            for ID_dir_abspath in jobs2launch:
                # Create the threads for every cluster
                setup_production_kwargs = {
                    'current_cluster':jobs2launch[ID_dir_abspath]['cluster'],
                ...
                cluster_process[jobs2launch[ID_dir_abspath]['cluster']] =  multiprocessing.Process(target = setup_production, kwargs=setup_production_kwargs)
            ...
            if frontend:
                main_process.start()
                main_process.join()
                for cluster in cluster_process:
                    cluster_process[cluster].start()
                    cluster_process[cluster].join()
            else:
                # Start the threads
                main_process.start()
                [cluster_process[cluster].start() for cluster in cluster_process]
                # print(f'Number of theads before the join (after the start): {threading.activeCount()}')
                main_process.join()
                [cluster_process[cluster].join() for cluster in cluster_process]
