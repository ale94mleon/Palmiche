...
def main(input_path_dict,
         ...
         number_frame={'pulling': 2000,
                       'equilibration': 2, 'production': 2000},
         temperature = 303.15,
         annealing = True,
    ...
    if pull_code:
        ...
        tools.run(f"gmx grompp -f pull.mdp -c {input_path_dict['conf']} -r {input_path_dict['conf']} -p {os.path.basename(input_path_dict['topol'])} -n {out_index_name} -o pull.tpr")
