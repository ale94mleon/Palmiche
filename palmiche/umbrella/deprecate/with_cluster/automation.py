...
def main(input_path_dict,
        ...
        simulation_time={'pulling': 100000,
                        'equilibration': 2000,
                        'annealing_for_ST':5000, # For now this is irrelevant because we are using the PandeWeights and for that the methodology is already fixed. This will be usefull in the case of the use of AleWeights
                        'production': 20000}, # ps
    ...
    simulation_time : TYPE, optional, dict
        DESCRIPTION. The default is {'pulling':100000,'equilibration':1000,'production':10000}. [ps]
    ...
    # Here I deffine the variable names for the index.
    out_index_name = 'index.ndx'

    window_path = os.path.join(output_path, 'windows')
    new_path_dict = {}
    for key in input_path_dict:
        if key != 'conf': # I don't wnat to copy this file to the directory, so, I need to keep the original path.
            new_path_dict[key] = os.path.join(
                output_path, os.path.basename(input_path_dict[key]))
    ...
    if pull_code:
        ...
        tools.run(
            f"export GMX_MAXBACKUP=-1; echo q | gmx make_ndx -f {input_path_dict['conf']} -o {tmpndx1.name}")
        options = f"""
        "SOLU" group Protein;
        ...
        with open(tmpopt1.name, 'w') as f:
            f.write(options)
        tools.run(
            f"export GMX_MAXBACKUP=-1; gmx select -s {input_path_dict['conf']} -sf {tmpopt1.name} -n {tmpndx1.name} -on {tmpndx2.name}")
        ...
        with open(tmpopt2.name, 'w') as f:
            f.write(options)
        tools.run(
            f"export GMX_MAXBACKUP=-1; gmx select -s {input_path_dict['conf']} -sf {tmpopt2.name} -n {tmpndx3.name} -on {tmpndx4.name}")
        ...
        JOBSH_pull = jobsh.JOB(
            ...
            build_GROMACS_section=f"{str_simulated_tempering_alpha}\n"\
                f"gmx grompp -f pull.mdp -c {input_path_dict['conf']} -r {input_path_dict['conf']} -p {os.path.basename(input_path_dict['topol'])} -n {out_index_name} -o pull.tpr\n"\
                f"gmx mdrun -nt {cpu} -cpi -stepout 5000 -v -deffnm pull -px pull_pullx -pf pull_pullf {update_gpu} >& pull.lis\n",
        )
        JOBSH_pull.write()
    ...
    if window_code:
        ...
        if simulated_tempering:
            ...
            out_string = f"{'window_ID':<15}{'frame':<10}{'dist':<10}{'delta_dist':<15}{'interval':<15}{'width':<10}{'force_constant':<20}{'cluster':<15}{'closer_to_cluster_center':<15}\n"   
            for window_ID in windows:
                if windows[window_ID]['frame'] in frame_close_to_centroids.values():
                    out_string += f"{window_ID:<15}{windows[window_ID]['frame']:<10}{windows[window_ID]['dist']:<10.3}{windows[window_ID]['delta_dist']:<15}{windows[window_ID]['interval']:<15}{windows[window_ID]['width']:<10.3}{windows[window_ID]['force_constant']:<25}{windows[window_ID]['cluster']:<15}{'X':<15}\n"
                else:
                    out_string += f"{window_ID:<15}{windows[window_ID]['frame']:<10}{windows[window_ID]['dist']:<10.3}{windows[window_ID]['delta_dist']:<15}{windows[window_ID]['interval']:<15}{windows[window_ID]['width']:<10.3}{windows[window_ID]['force_constant']:<25}{windows[window_ID]['cluster']:<15}{' ':<15}\n"
        else:
            out_string = f"{'window_ID':<15}{'frame':<10}{'dist':<10}{'delta_dist':<15}{'interval':<15}{'width':<10}{'force_constant':<10}\n"   
            for window_ID in windows:
                out_string += f"{window_ID:<15}{windows[window_ID]['frame']:<10}{windows[window_ID]['dist']:<10.3}{windows[window_ID]['delta_dist']:<15}{windows[window_ID]['interval']:<15}{windows[window_ID]['width']:<10.3}{windows[window_ID]['force_constant']:<15}\n"
        ...
        if simulated_tempering:
            ...
            for cluster in frame_close_to_centroids:
                ...
                if test:
                    annealing_for_ST_temp, annealing_for_ST_time = MDP_annealing_for_ST.annealing3(NumbOfGroups = len(tc_grps), temp = np.arange(temperature, temperature + 32, 2), heating_time=2, constant_temp_time=2)
                else:
                    annealing_for_ST_temp, annealing_for_ST_time = MDP_annealing_for_ST.annealing3(NumbOfGroups = len(tc_grps), temp = np.arange(temperature, temperature + 32, 2), heating_time=100, constant_temp_time=2000)
                ...
                JOBSH_equilibration_annealing_for_ST = jobsh.JOB(
                    ...
                    build_GROMACS_section= f"{str_simulated_tempering_alpha}\n"\
                        ...
                        f"gmx mdrun -nt {cpu} -cpi -stepout 5000 -v -deffnm equilibration -px equilibration_pullx -pf equilibration_pullf {update_gpu} >& equilibration.lis\n\n\n"\
                        f"gmx grompp -f annealing_for_ST.mdp -o annealing_for_ST.tpr -c equilibration.gro -r {conf} -p {os.path.basename(new_path_dict['topol'])} -n {os.path.basename(new_path_dict['ndx'])}\n"\
                        f"gmx mdrun -nt {cpu} -cpi -stepout 5000 -v -deffnm annealing_for_ST -px annealing_for_ST_pullx -pf annealing_for_ST_pullf {update_gpu} >& annealing_for_ST.lis\n"
                )
            ...
            if frontend:
                JOBIDs = tools.job_launch_list([os.path.join(path, '1_job.sh') for path in jobs2launch], shell='bash')# This is just for testing
            else: 
                # This will launch and reapeat (jobsh_name) in case that logfile did not finish correctlly due to runing out of tiem on the selected partition, 
                tools.launch_wait_check_repeat(
                    partition = partition,
                    jobpaths = list(jobs2launch.keys()),
            ...
            for ID_dir_abspath in jobs2launch:
                ...
                MDP_production.simulated_tempering(
                    temp_list = np.arange(temperature, temperature + 32, 2), # The same used for the anealing for the prediction of the inital weights
                ...
                JOBSH_production = jobsh.JOB(
                    ...
                    build_GROMACS_section=f"{str_simulated_tempering_alpha}\n"\
                        ...
                        f"gmx mdrun -nt {cpu} -cpi -stepout 5000 -v -deffnm production -px production_pullx -pf production_pullf >& production.lis\n"
                )
                JOBSH_production.write(os.path.join(ID_dir_abspath,'2_job.sh'))
            # Launch the jobs and wait for end of execution of all center of windows: DONE!
            if frontend:
                JOBIDs = tools.job_launch_list([os.path.join(path, '2_job.sh') for path in jobs2launch], shell='bash')# This is just for testing
            else:
                # This will launch and reapeat (jobsh_name) in case that logfile did not finish correctlly due to runing out of tiem on the selected partition, 
                tools.launch_wait_check_repeat(
                    partition = partition,
            ...
            calculated_initial_weights = {} # It will have as key the cluster ID and as value the array of weights
            for ID_dir_abspath in jobs2launch:
                time_progression, weights_info = weights.get_weights_from_log(os.path.join(ID_dir_abspath,'production.log'), plot = True)
                calculated_initial_weights[jobs2launch[ID_dir_abspath]['cluster']] = {
                     'time': time_progression,
                     'weights_info': weights_info,
                 }
                 # Here I could draw the probability flow matrix for the center of clusters
            
            # Loops for the rest of the windows and assign the corresponded weights depdending on what cluster belongs
            # Initialize jobs2launch and get the new jobs to launch
            jobs2launch = {}
            for cluster in user_membership:
                for frame in user_membership[cluster]:
                    ...
                    MDP_production.simulated_tempering(
                        temp_list = np.arange(temperature, temperature + 32, 2), # The same used for the anealing for the prediction of the inital weights
                        init_lambda_weights = calculated_initial_weights[cluster]['weights_info'][-1,:,1], # Take final weights and assign them to their respective members of cluster
                    )
                    MDP_production.write(os.path.join(ID_dir_abspath,'production.mdp'))
                    ...
                    JOBSH_production.write(os.path.join(ID_dir_abspath,'3_job.sh'))

            # Launch the jobs and wait for end of execution
            if frontend:
                JOBIDs = tools.job_launch_list([os.path.join(path, '3_job.sh') for path in jobs2launch], shell='bash')# This is just for testing
            else:
                # This will launch and reapeat (jobsh_name) in case that logfile did not finish correctlly due to runing out of tiem on the selected partition, 
                tools.launch_wait_check_repeat(
                    partition = partition,
            ...
            for window_ID in windows:
                ...
                _, weights_info = weights.get_weights_from_log(os.path.join(ID_dir_abspath,'production.log'), plot = True)
            ...
            for window_ID in windows:
                ...
                JOBSH_window = jobsh.JOB(
                    ...
                    build_GROMACS_section=f"{str_simulated_tempering_alpha}\n"\
                        f"gmx grompp -f equilibration.mdp -o equilibration.tpr -c {conf} -r {conf} -p {os.path.basename(new_path_dict['topol'])} -n {os.path.basename(new_path_dict['ndx'])}\n"\
                        f"gmx mdrun -nt {cpu} -cpi -stepout 5000 -v -deffnm equilibration -px equilibration_pullx -pf equilibration_pullf {update_gpu} >& equilibration.lis\n\n\n"\
                        f"gmx grompp -f production.mdp -o production.tpr -c equilibration.gro -r {conf} -p {os.path.basename(new_path_dict['topol'])} -n {os.path.basename(new_path_dict['ndx'])}\n"\
                        f"gmx mdrun -nt {cpu} -cpi -stepout 5000 -v -deffnm production -px production_pullx -pf production_pullf {update_gpu} >& production.lis\n"
                )
